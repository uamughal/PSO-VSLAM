...
% fitness function
function [S_Region_p_s,bestpa,fit]=fitness(par,SAI)
...
%       region.RS(r,col)= RS;
    ...
    S_Region_p_s = [S_Region_p_s; row col RS]; 
    save('S_Region(p_s).mat','S_Region_p_s')

%       sense1=cat(1,region.RS);
%           sense.sen=cat(1,region.RS); % // stored value in sense struct
          
 end
 RSC=RSC+rsc;
%  sense.RSC=RSC; %// sense struct
 s=SAI(t(1),t(2))+SAI(t(1),t(2)+1)+SAI(t(1)+1,t(2))+SAI(t(1)+1,t(2)+1);
    sai=sai+s;
end
